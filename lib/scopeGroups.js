...
// ── Job naming ─────────────────────────────────────────────────────
// Jobs sync from GoHighLevel named after the opportunity, which is just the
// customer's first and last name. A job needs a project word after the name so
// a customer with two jobs is tellable apart in the filing cabinet.
//
// JobTread caps job names at 30 characters, and existing names already sit at
// the ceiling ("Standish Tub to Shower Remodel" is exactly 30), so the suffix
// has to be short and the result has to be clamped.
const JOB_NAME_MAX_LENGTH = 30;

const PROJECT_WORDS = [
  'bath', 'bathroom', 'shower', 'tub', 'kitchen', 'deck', 'basement',
  'laundry', 'remodel', 'addition', 'roof', 'floor', 'flooring', 'closet',
  'patio', 'fence', 'garage', 'siding', 'window', 'door', 'porch', 'wet area',
  'utility', 'office', 'living', 'bedroom', 'exterior', 'interior', 'paint'
];

// True when the name looks like a bare person's name — no project word in it.
function needsProjectName(name) {
  if (!name) return false;
  const lower = name.toLowerCase();
  return !PROJECT_WORDS.some(word => lower.includes(word));
}

// The descriptors Elite actually uses, taken from 103 existing job names.
// "Bathroom" dominates; the rest appear repeatedly. Given to the model as a
// preferred vocabulary so new names match how the filing cabinet already reads.
const PROJECT_NAME_VOCABULARY = [
  'Bathroom', 'Master Bathroom', 'Shower', 'Shower Remodel', 'Tub to Shower',
  'Wet Area', 'Deck', 'Kitchen', 'Basement Finish', 'Laundry Room',
  'Flooring', 'Garage', 'Remodel'
];

// Shorter stand-ins, longest first, for when the full descriptor will not fit
// in 30 characters. Blindly taking the first word would turn "Tub to Shower"
// into "Tub", so each term names its own sensible shortening.
const PROJECT_NAME_FALLBACKS = {
  'master bathroom': ['Master Bathroom', 'Bathroom', 'Bath'],
  'bathroom':        ['Bathroom', 'Bath'],
  'tub to shower':   ['Tub to Shower', 'Shower'],
  'shower remodel':  ['Shower Remodel', 'Shower'],
  'basement finish': ['Basement Finish', 'Basement'],
  'laundry room':    ['Laundry Room', 'Laundry'],
  'wet area':        ['Wet Area'],
  'garage build':    ['Garage Build', 'Garage']
};

function projectNameVariants(descriptor) {
  const clean = String(descriptor || '').trim().replace(/\s+/g, ' ');
  if (!clean) return [];
  const known = PROJECT_NAME_FALLBACKS[clean.toLowerCase()];
  if (known) return known;
  const firstWord = clean.split(' ')[0];
  return clean === firstWord ? [clean] : [clean, firstWord];
}

// Appends the descriptor, clamped to JobTread's limit, trying progressively
// shorter forms. Returns the original name unchanged when nothing fits, so a
// rename never truncates the customer's actual name.
function appendProjectName(name, descriptor) {
  if (!name || !descriptor) return name;
  const base = name.trim();
  const fitting = projectNameVariants(descriptor)
    .map(variant => `${base} ${variant}`)
    .find(candidate => candidate.length <= JOB_NAME_MAX_LENGTH);
  return fitting || name;
}
