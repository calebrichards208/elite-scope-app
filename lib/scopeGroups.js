...
// Appends the descriptor, clamped to JobTread's limit. Returns the original
// name unchanged when there is no room, so a rename never truncates the
...
function appendProjectName(name, descriptor) {
  ...
  const combined = `${name.trim()} ${clean}`;
  if (combined.length <= JOB_NAME_MAX_LENGTH) return combined;

  // Not enough room for the full descriptor — try the first word of it.
  const firstWord = clean.split(' ')[0];
  const shorter   = `${name.trim()} ${firstWord}`;
  return shorter.length <= JOB_NAME_MAX_LENGTH ? shorter : name;
}
