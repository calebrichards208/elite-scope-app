...
function documentLineItems(mode, groupName, postedItems, notes) {
  const group = {
    ...
    lineItems: (postedItems || []).map(item => {
      // unitCost must be set — JobTread refuses to approve a document with an
      // unset cost on any line ("A cost must be set for X"). Sergey prices
      // lump-sum and tracks no costs, so 0 is the honest value.
      ...
      // 0 into document copies; the API leaves them null unless told.
      const line = {
